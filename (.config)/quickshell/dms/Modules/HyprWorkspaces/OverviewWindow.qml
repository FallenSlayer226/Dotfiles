...
Item {
    ...
    // Fix: Use Paths.moddedAppId for consistent app ID handling
    property string appClass: windowData?.class ?? ""
    property string moddedAppId: Paths.moddedAppId(appClass)
    property var entry: DesktopEntries.heuristicLookup(moddedAppId)
    
    property var iconPath: Quickshell.iconPath(entry?.icon ?? appClass ?? "application-x-executable", "image-missing")
